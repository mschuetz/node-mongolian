...
function parseUrl(urlString) {
	...
	if (url.protocol != 'mongo:' && url.protocol != 'mongodb:')
		throw new Error("Invalid protocol : " + url.protocol);
	var mongoUrl = {}
	if (url.auth) {
		authComponents = url.auth.split(':')
		mongoUrl.user = authComponents[0]
		mongoUrl.password = authComponents[1]
	}
	mongoUrl.host = url.hostname
	mongoUrl.port = url.port || 27017
	if (url.pathname.length == 0 || url.pathname[0] != '/')
		throw new Error("Invalid database in connection URL: " + urlString);
	
	mongoUrl.database = decodeURIComponent(url.pathname).slice(1)
	return mongoUrl
}
